...
// ============================================================
const START_DATE = '2026-01-23';  // Start date (inclusive)
const END_DATE = '2026-02-08';    // End date (inclusive)
// ============================================================

/**
 * Update connected_at for kaia_2048_users based on daily targets
 * Each date updates a number of records equal to the difference from the previous date
 */
async function updateConnectedDates() {
  try {
    ...
    console.log('🚀 Starting to update connected_at for kaia_2048_users');
    ...
    const checkQuery = `
      ...
      WHERE connected_at IS NULL AND platform = 'tofu'
    `;
    const checkResult = await pool.query(checkQuery);
    const availableRecords = parseInt(checkResult.rows[0].count);
    console.log(`📊 Available records to update: ${availableRecords.toLocaleString()}\n`);
    ...
    const verifyQuery = `
      SELECT 
        ...
        COUNT(*) as count
      FROM kaia_2048_users
      WHERE platform = 'tofu' AND connected_at IS NOT NULL
      GROUP BY connected_at
      ORDER BY connected_at
    `;
    
    console.log('📋 Verification - Records per date:');
    ...
    console.log(`\n   Remaining NULL records: ${remainingRecords.toLocaleString()}\n`);
    
  } catch (error) {
    console.error('❌ Error updating connected dates:', error);
    throw error;
  } finally {
    await pool.end();
